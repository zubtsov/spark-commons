...
object DataFrameComparison {
  ...
  object ColumnNamesDifference {
    def get(leftColumns: Seq[String], rightColumns: Seq[String], caseSensitive: Boolean = defaultCaseSensitivity): ColumnNamesDifference = {
  ...
  object SchemaDifference {
    def get(left: StructType, right: StructType, ignoreNullability: Boolean = true, caseSensitive: Boolean = defaultCaseSensitivity): SchemaDifference = {
      val sc = areStringsEqual(caseSensitive)(_, _)
  ...
  object DataDifference {
    def get(left: DataFrame, right: DataFrame): DataDifference = {
    ...
    def getApproximate(left: DataFrame, right: DataFrame, primaryKey: Seq[String], numericColToAbsError: Map[String, Double]): DataDifference = {
      getApproximate2(left, right, primaryKey, numericColToAbsError.mapValues(v => lit(v)))
    }

    def getApproximate(left: DataFrame, right: DataFrame, numericColToAbsError: Map[String, Double]): DataDifference = {
      getApproximate(left, right, left.drop(numericColToAbsError.keys.toSeq:_*).columns, numericColToAbsError)
    }

    def getApproximate(left: DataFrame, right: DataFrame, numericCols: Seq[String], absoluteError: Double): DataDifference = {
      getApproximate(left, right, numericCols.map(cn => (cn, absoluteError)).toMap)
    }

    def getApproximate2(left: DataFrame, right: DataFrame, primaryKey: Seq[String], numericColToAbsError: Map[String, Column]): DataDifference = {
  ...
  def hasEqualColumnNames(leftColumns: Seq[String], rightColumns: Seq[String], caseSensitive: Boolean = defaultCaseSensitivity): Boolean = {
    ColumnNamesDifference.get(leftColumns, rightColumns, caseSensitive).isTheSame
  }
  ...
  def hasDifferentColumnNames(leftColumns: Seq[String], rightColumns: Seq[String], caseSensitive: Boolean = defaultCaseSensitivity): Boolean = {
    ColumnNamesDifference.get(leftColumns, rightColumns, caseSensitive).isDifferent
  }
  ...
  def hasEqualSchemas(left: DataFrame, right: DataFrame, ignoreNullability: Boolean = true, caseSensitive: Boolean = defaultCaseSensitivity): Boolean = {
    SchemaDifference.get(left.schema, right.schema, ignoreNullability, caseSensitive).isTheSame
  }
  ...
  def hasDifferentSchemas(left: DataFrame, right: DataFrame, ignoreNullability: Boolean = true, caseSensitive: Boolean = defaultCaseSensitivity): Boolean = {
    SchemaDifference.get(left.schema, right.schema, ignoreNullability, caseSensitive).isDifferent
  }

  def hasEqualData(left: DataFrame, right: DataFrame): Boolean = {
    DataDifference.get(left, right).isTheSame
  }

  def hasDifferentData(left: DataFrame, right: DataFrame): Boolean = {
    DataDifference.get(left, right).isDifferent
  }

  def assertEqualColumnNames(left: DataFrame, right: DataFrame, caseSensitive: Boolean = defaultCaseSensitivity): Unit = {
    val columnNamesDifference = ColumnNamesDifference.get(left.columns, right.columns, caseSensitive)
  ...
  def assertEqualSchemas(left: DataFrame, right: DataFrame, ignoreNullability: Boolean = true, caseSensitive: Boolean = defaultCaseSensitivity): Unit = {
    val schemaDifference = SchemaDifference.get(left.schema, right.schema, ignoreNullability, caseSensitive)
  ...
  def assertEqualData(left: DataFrame, right: DataFrame): Unit = {
    val dataDifference = DataDifference.get(left, right)
