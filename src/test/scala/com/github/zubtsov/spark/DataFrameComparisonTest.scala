...
class DataFrameComparisonTest extends SparkFunSuite {
  test("Column names difference case insensitive") {
    ...
    val difference = DataFrameComparison.ColumnNamesDifference.get(df1.columns, df2.columns, caseSensitive = false)
  ...
  test("Column names difference case sensitive") {
    val df1 = spark.emptyDataFrame2("col1 STRING, col2 STRING, col3 DOUBLE, col4 INT")
    val df2 = spark.emptyDataFrame2("col1 INT, COL2 INT, COL3 DOUBLE, col5 INT")
    val difference = DataFrameComparison.ColumnNamesDifference.get(df1.columns, df2.columns, caseSensitive = true)
    assert(difference.isDifferent)
  ...
  test("Schema difference case insensitive") {
    val df1 = spark.emptyDataFrame2("col1 STRING, col2 STRING, col3 DOUBLE, col4 INT")
    val df2 = spark.emptyDataFrame2("col1 INT, COL2 INT, COL3 DOUBLE, col5 INT")
    val difference = DataFrameComparison.SchemaDifference.get(df1.schema, df2.schema, caseSensitive = false)
  ...
  test("Schema difference case sensitive") {
    ...
    val difference = DataFrameComparison.SchemaDifference.get(df1.schema, df2.schema, caseSensitive = true)
  ...
  test("Data difference") {
    ...
    val difference = DataFrameComparison.DataDifference.get(df1, df2)
  ...
  test("Data difference approximate") {
    ...
    val difference = DataFrameComparison.DataDifference.getApproximate(df1, df2, Seq("double_val", "decimal_val"), 0.05)
  ...
  test("Data difference approximate 2") {
    ...
    val difference = DataFrameComparison.DataDifference.getApproximate(df1, df2, Seq("string_val"), Map("double_val" -> 0.6, "decimal_val" -> 0.05))
    assert(difference.leftMissingRows.collect().toSet == Set(
    ...
    val difference2 = DataFrameComparison.DataDifference.getApproximate(df1, df2, Seq("string_val"), Map("double_val" -> 0.5, "decimal_val" -> 0.06))
